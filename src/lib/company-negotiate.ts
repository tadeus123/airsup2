...
function systemPrompt(company: CompanySecret, visitorUsername: string, isOwnerTest: boolean): string {
  const stance = company.stance.trim() ||
  ...
  const testLine = isOwnerTest
    ? "The other side is the company owner testing you, playing a buyer. Still negotiate for real so they can hear how you sound."
  ...
  const messages = [
    {
      ...
      content: systemPrompt(input.company, input.visitorUsername, isOwnerTest),
    },
    ...prior,
